...
Ein dauerhaftes Seitenpanel dominiert die Fläche, die eigentlich der Canvas braucht. Lösung: Das **Whiteboard-Log** ist standardmäßig aus (`panelOpenFlag`), öffnet sich ~5 s, wenn eine **Agentenaktion** ausgeführt wurde (`showPanelBriefly`, `panelAutoUntil`), und schließt sich dann selbst — der Mensch kann es über `📋 Log einblenden` dauerhaft anheften. Zwei Details waren nötig, damit das trägt: (a) nach Ablauf des Fensters muss ein Timer ein Re-Render anstoßen, sonst bleibt das Log stehen; (b) die Einblendung muss am **Aktionskontext** hängen, nicht am Objekt-Akteur.

Die erste Fassung leitete die Einblendung aus dem Snapshot-Diff ab (`n.actor === 'agent' || n.movedBy === 'agent'` → Suffix „ (Agent)"). Das war falsch: Der Akteur beschreibt den **Ersteller** eines Objekts. Verschiebt oder ändert der Mensch einen Zettel, den zuvor der Renderer angelegt hat, trägt dieser weiterhin `actor: 'agent'` — der Diff meldete „(Agent)" und das Log blendete sich für eine rein menschliche Handlung ein. Die Attribution im Diff bleibt deshalb als reine Textauszeichnung akteurssensitiv, die Einblendung aber kommt ausschließlich aus der soeben ausgeführten Aktion: `executedAgentActions` zählt nur Befehle, die wirklich über `execCommand` liefen (ein replayed Ack zählt nicht); nur dann zeigt `recordChanges(..., true)` / `showPanelBriefly` das Log. Eine menschliche Aktion an einem Renderer-Objekt wird protokolliert, blendet aber nie ein.
