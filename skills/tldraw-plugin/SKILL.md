...
- Seitenpanel standardmäßig **aus** und nur bei Agentenänderungen kurz zeigen: `panelOpenFlag` (Mensch heftet an) + `panelAutoUntil` (Auto-Fenster, z. B. 5 s); sichtbar = `flag || Date.now() < autoUntil`. Nach Ablauf ein `pushUi()` per Timer planen, sonst rendert nichts neu und das Panel bleibt stehen; manuelles Umschalten setzt `panelAutoUntil = 0`.
- Den Snapshot-Diff **akteurssensitiv** machen (`n.actor === 'agent' || n.movedBy === 'agent'` → Suffix „ (Agent)"). Sonst erkennt die Auto-Einblendung Agenten-Verschiebungen und Reparentings nicht, weil nur neue Zettel den Akteur tragen.
