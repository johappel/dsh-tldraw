...
- **Whiteboard-Log statt Dauer-Panel** — standardmäßig ausgeblendet; nur eine wirklich ausgeführte **Agentenaktion** am Board blendet es ~5 s ein und danach wieder aus. Eine menschliche Aktion blendet es **nie** ein — auch dann nicht, wenn sie ein Objekt betrifft, das zuvor der Renderer angelegt hat; sie wird nur protokolliert. Der Kopfzeilen-Knopf `📋` heftet es dauerhaft an und löst die Heftung wieder; er leuchtet, solange das Log sichtbar ist
- **Gestaltungsleiste per Knopf** — standardmäßig **ausgeblendet**; der Kopfzeilen-Knopf `🎨` schaltet die tldraw-Leiste oben rechts (Farbe, Strichstärke, Deckkraft, Schrift, Ausrichtung, Rahmen). Der Schalter ist rein menschlich: Er folgt weder Agentenaktionen noch dem Whiteboard-Log, ist unabhängig vom Log-Knopf und ändert nur die Sichtbarkeit im bestehenden Slot — kein neuer Editor, kein Store-Schreibvorgang. In einer Sidebar bis 640 px Breite zeichnet tldraw diese Leiste gar nicht; dort bleibt tldraws eigener Kompakt-Knopf in der Werkzeugleiste zuständig. Eingeschaltet, aber ohne aktives Zeichenwerkzeug und ohne Auswahl, ist die Leiste legitim leer
- **Zwei Icon-Schalter statt Textknöpfe** — `🎨` und `📋` sind die einzigen Knöpfe der Kopfzeile ohne Beschriftungstext, damit die Zeile in einer schmalen Sidebar nicht umbricht. Die Worte bleiben vollständig erhalten: als `aria-label` (Name für Screenreader), als `title` (Tooltip mit dem ganzen Satz) und als `aria-pressed` (an/aus). Beide Knöpfe leuchten im aktiven Zustand (`wb-btn-on`)
